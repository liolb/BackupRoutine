...
class BackupManager():
    ...
    def _do_backup(self, profiles: list[Profile], destinations: list[Destination]) -> None:
        from .utils import filter_files, create_zip, copy_file
    ...
    def run(self) -> int:  #
        ...
        # #Krücke: How to notify the user that an error occured? 
        if os.path.exists(self.log.get_errorlog_file()):
            from .utils import open_with_editor
            open_with_editor(self.log.get_errorlog_file())  
        else:
            self.log.log_hint('No errors occured.')      
